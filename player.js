...
(function(global) {
  ...
  class StellarPlayer {
    ...
    //   • Anime contexts see ONLY anime kinds (megaplay + scraped embeds).
    //   • Movie / TV contexts see everything EXCEPT those anime kinds.
    // Keeps defaultChain() as a single registry while preventing cross-type leakage.
    _effectiveServers() {
      const all = this.opts.servers || [];
      const isAnime = this.ctx && this.ctx.type === 'anime';
      const ANIME_KINDS = ['megaplay', 'embed'];
      return all.filter(s => isAnime ? ANIME_KINDS.includes(s.kind) : !ANIME_KINDS.includes(s.kind));
    }
