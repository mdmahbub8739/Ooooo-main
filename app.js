...
if ('navigation' in window) {
  ...
  function _hexToRgba(hex, a) {
    ...
    if (!m) return null;
    const r = parseInt(m[0].slice(0,2),16), g = parseInt(m[0].slice(2,4),16), b = parseInt(m[0].slice(4,6),16);
    return `rgba(${r},${g},${b},${a})`;
  }
  let _perShowAccentActive = false;
  function applyPerShowAccent(hex) {
    const soft = _hexToRgba(hex, 0.18);
    const glow = _hexToRgba(hex, 0.55);
    if (!soft || !glow) return;
    document.documentElement.style.setProperty('--accent', hex.startsWith('#') ? hex : '#' + hex);
    document.documentElement.style.setProperty('--accent-soft', soft);
    document.documentElement.style.setProperty('--accent-glow', glow);
    document.documentElement.classList.add('per-show-accent');
    _perShowAccentActive = true;
  }
  function resetPerShowAccent() {
    if (!_perShowAccentActive) return;
    _perShowAccentActive = false;
    document.documentElement.classList.remove('per-show-accent');
    const name = localStorage.getItem(STORE_ACCENT);
    const a = ACCENTS.find(x => x.name === name) || ACCENTS[0];
    document.documentElement.style.setProperty('--accent', a.accent);
    document.documentElement.style.setProperty('--accent-soft', a.soft);
  ...
  const STORE_AMOLED = 'animowebAmoled';
  function amoledEnabled() { return localStorage.getItem(STORE_AMOLED) === 'on'; }
  function setAmoledEnabled(on) {
    localStorage.setItem(STORE_AMOLED, on ? 'on' : 'off');
    document.documentElement.classList.toggle('theme-amoled', !!on);
  }
  document.documentElement.classList.toggle('theme-amoled', amoledEnabled());

  const STORE_PER_SHOW_ACCENT = 'animowebPerShowAccent';
  function perShowAccentEnabled() {
    const v = localStorage.getItem(STORE_PER_SHOW_ACCENT);
    return v === null ? false : v === 'on';
  }
  function setPerShowAccentEnabled(on) {
    localStorage.setItem(STORE_PER_SHOW_ACCENT, on ? 'on' : 'off');
    if (!on) resetPerShowAccent();
  }

  const _animeEpisodeCache = {};
  function _cacheEpisodeLinks(anilistId, ep, workerName, links) {
    const a = _animeEpisodeCache[anilistId] = _animeEpisodeCache[anilistId] || {};
    const e = a[ep] = a[ep] || {};
    e[workerName] = links || [];
  }
  function _getCachedEpisodeLinks(anilistId, ep, workerName) {
    return _animeEpisodeCache?.[anilistId]?.[ep]?.[workerName];
  }
  function _clearEpisodeCache(anilistId) {
    if (anilistId == null) { for (const k of Object.keys(_animeEpisodeCache)) delete _animeEpisodeCache[k]; }
    else delete _animeEpisodeCache[anilistId];
  }

  const _animeChainCache = new Map();
  async function _fetchAniRelations(aniId) {
    try {
        ...
        body: JSON.stringify({
          query: `query($id:Int){
            Media(id:$id, type:ANIME){
              relations{ edges{ relationType node{ id type format episodes seasonYear
      ...
      return j?.data?.Media?.relations?.edges || [];
    } catch (_) { return []; }
  }
  async function _buildSeasonChain(rootId, rootMedia) {
    const cached = _animeChainCache.get(rootId);
    if (cached) return cached;
    const VALID_RELS = ['PREQUEL','SEQUEL','PARENT','SIDE_STORY'];
    const VALID_FORMATS = ['TV','TV_SHORT','ONA','OVA','SPECIAL','MOVIE'];
    const map = new Map();
    map.set(rootId, {
      id: rootId,
      title: rootMedia.title?.english || rootMedia.title?.romaji || 'Unknown',
      episodes: rootMedia.episodes,
      seasonYear: rootMedia.seasonYear,
      format: rootMedia.format,
      cover: rootMedia.coverImage?.large || '',
    });
    let frontier = [rootId];
    for (let depth = 0; depth < 3 && frontier.length; depth++) {
      const results = await Promise.all(frontier.map(_fetchAniRelations));
      const next = [];
      for (const edges of results) {
        for (const e of edges) {
          if (!VALID_RELS.includes(e.relationType)) continue;
          if (!e.node || e.node.type !== 'ANIME') continue;
          if (!VALID_FORMATS.includes(e.node.format)) continue;
          if (map.has(e.node.id)) continue;
          map.set(e.node.id, {
            id: e.node.id,
    ...
    const chain = [...map.values()].sort((a, b) =>
      (a.seasonYear || 9999) - (b.seasonYear || 9999) || a.id - b.id
    );
    for (const s of chain) _animeChainCache.set(s.id, chain);
    return chain;
  }
  ...
  async function route() {
    ...
    if (!(name === 'watch' && args[0] === 'anime')) resetPerShowAccent();
  ...
  const mini = {
    ...
    expand() {
      ...
      const path = ctx.type === 'anime'
        ? `/watch/anime/${ctx.id}/${ctx.episode || 1}`
        : ctx.type === 'tv'
  ...
  async function renderWatch(...args) {
    if (args[0] === 'anime') return renderAnimeWatch(args[1], args[2] || 1);
  ...
  // ── Anime scraper-worker toolkit ────────────────────────────────────────────
  // Auto title-matching produced wrong hits, so the user MANUALLY picks the
  // correct entry once per worker. We remember that choice per AniList id, then
  // resolve every episode from the chosen entry's URL (no re-searching, no
  // mismatches). MegaPlay always plays by AniList id and needs no picking.
  const _animeMatch = {}; // { [anilistId]: { [workerName]: { title, url } } }

  // Persist user's manual title picks across reloads so they never re-search.
  const _MATCH_LS_KEY = 'animoweb_anime_match_v1';
  try {
    const raw = localStorage.getItem(_MATCH_LS_KEY);
    if (raw) Object.assign(_animeMatch, JSON.parse(raw));
  } catch (_) {}
  function _saveAnimeMatches() {
    try { localStorage.setItem(_MATCH_LS_KEY, JSON.stringify(_animeMatch)); } catch (_) {}
  }

  function _stripSeasonSlug(title) {
    if (!title) return title;
    let t = String(title);
    t = t.replace(/\s*(?::|·|-|–|—)\s*(?:season|s)\s*\d+\b.*$/i, '');
    t = t.replace(/\s*(?::|·|-|–|—)\s*\d+(?:st|nd|rd|th)\s*season\b.*$/i, '');
    t = t.replace(/\s*(?::|·|-|–|—)\s*part\s*\d+\b.*$/i, '');
    t = t.replace(/\s*(?::|·|-|–|—)\s*cour\s*\d+\b.*$/i, '');
    t = t.replace(/\s+season\s+\d+\s*$/i, '');
    t = t.replace(/\s+s\d+\s*$/i, '');
    t = t.replace(/\s+\d+(?:st|nd|rd|th)\s+season\s*$/i, '');
    t = t.replace(/\s+part\s+\d+\s*$/i, '');
    t = t.replace(/\s+cour\s+\d+\s*$/i, '');
    t = t.replace(/\s+(?:ii|iii|iv|v|vi|vii|viii|ix|x)\s*$/i, '');
    return t.trim() || title;
  }

  const _ROMAN_SEASON = { ii:2, iii:3, iv:4, v:5, vi:6, vii:7, viii:8, ix:9, x:10 };

  function _detectAnimeSeason(title) {
    if (!title) return null;
    const t = String(title);
    let m;
    if ((m = t.match(/(?:season|s)\s*(\d{1,2})\b/i))) return parseInt(m[1], 10);
    if ((m = t.match(/(\d{1,2})(?:st|nd|rd|th)\s*season\b/i))) return parseInt(m[1], 10);
    if ((m = t.match(/\bpart\s*(\d{1,2})\b/i))) return parseInt(m[1], 10);
    if ((m = t.match(/\bcour\s*(\d{1,2})\b/i))) return parseInt(m[1], 10);
    if ((m = t.match(/\s(ii|iii|iv|v|vi|vii|viii|ix|x)\s*$/i))) return _ROMAN_SEASON[m[1].toLowerCase()] || null;
    return null;
  }

  function _pickAnimeEpisode(episodes, season, episode) {
    if (!Array.isArray(episodes) || !episodes.length) return null;
    const wantEp = parseInt(episode, 10);
    const wantSe = (season == null || season === '') ? null : parseInt(season, 10);
    const eps = episodes
      .filter(e => e && e.url)
      .map(e => ({
        ...e,
        season: parseInt(e.season, 10) || 1,
        episode: parseInt(e.episode, 10) || 0,
        abs: (e.abs_episode != null ? parseInt(e.abs_episode, 10) : parseInt(e.episode, 10)) || 0,
      }))
      .sort((a, b) => (a.season - b.season) || (a.episode - b.episode));
    if (!eps.length) return null;
    const seasons = [...new Set(eps.map(e => e.season))];
    if (wantSe != null) {
      const exact = eps.find(e => e.season === wantSe && e.episode === wantEp);
      if (exact) return exact;
      const exactAbs = eps.find(e => e.season === wantSe && e.abs === wantEp);
      if (exactAbs) return exactAbs;
    }
    const byAbs = eps.find(e => e.abs === wantEp);
    if (byAbs && (wantSe == null || seasons.length === 1)) return byAbs;
    if (seasons.length === 1) {
      const only = eps.find(e => e.episode === wantEp);
      if (only) return only;
    }
    if (wantSe != null && seasons.includes(wantSe)) {
      const within = eps.filter(e => e.season === wantSe);
      const byNum = within.find(e => e.episode === wantEp);
      if (byNum) return byNum;
      if (within[wantEp - 1]) return within[wantEp - 1];
    }
    if (byAbs) return byAbs;
    if (eps[wantEp - 1]) return eps[wantEp - 1];
    return eps.find(e => e.episode === wantEp) || null;
  }

  // Auto-pick the best candidate from a worker's /search result for `title`.
  // Returns the chosen { title, url, ... } or null. Heuristic: exact > prefix > contains.
  function _autoPickAnimeMatch(candidates, wantTitle) {
    ...
    for (const c of candidates) {
      ...
      if (!t) continue;
      if (t === want && !exact) exact = c;
      else if (t.startsWith(want) && !prefix) prefix = c;
      else if (t.includes(want) && !contains) contains = c;
    }
    return exact || prefix || contains || null;
  }

  // GET /search?query= → candidate list [{ title, url, poster, type }]
  async function searchAnimeWorker(worker, query) {
    try {
      if (!worker || !worker.url) return [];
      const base = String(worker.url).replace(/\/$/, '');
      const r = await fetch(`${base}/search?query=${encodeURIComponent(query)}`);
      if (!r.ok) return [];
      const j = await r.json();
      return (j && j.status === 'success' && Array.isArray(j.data)) ? j.data : [];
    } catch (_) { return []; }
  }

  // Resolve THIS episode's stream URLs from a worker. `match` is what the user
  // picked in the match modal: { title, url }. Two worker API styles supported
  // (set per-worker via `api` in config.js ANIME_WORKERS):
  //
  //   'legacy' (e.g. ToonStream): GET /play?title=&season=&episode=
  //            → { status, embed_links:["url", ...] }
  //   'url'    (e.g. 4k AnimeSalt): GET /details?url=<series> to find the
  //            episode URL, then GET /play?url=<episode> → { status, data:[{name,url}] }
  //
  // Always returns a flat array of plain stream-URL strings (or [] on failure).
  async function resolveAnimeWorker(worker, match, season, episode) {
    try {
      if (!worker || !worker.url) return [];
      const base = String(worker.url).replace(/\/$/, '');
      // Backward-compat: `match` used to be a bare title string.
      const matchTitle = (match && typeof match === 'object') ? match.title : match;
      let matchUrl = (match && typeof match === 'object') ? match.url : '';
      const api = String(worker.api || 'legacy').toLowerCase();

      if (api === 'url') {
        // 1. Resolve the series/movie URL (use the remembered one, else re-search).
        if (!matchUrl && matchTitle) {
          const cands = await searchAnimeWorker(worker, matchTitle);
          const want = String(matchTitle).toLowerCase();
          const hit = cands.find(c => (c.title || '').trim().toLowerCase() === want) || cands[0];
          matchUrl = hit && hit.url;
        }
        if (!matchUrl) return [];

        // 2. /details → locate this episode's URL (movies play straight from the URL).
        let episodeUrl = matchUrl;
        const dr = await fetch(`${base}/details?url=${encodeURIComponent(matchUrl)}`);
        if (!dr.ok) return [];
        const dj = await dr.json();
        const data = (dj && dj.status === 'success' && dj.data) ? dj.data : null;
        if (!data) return [];

        if (data.is_series && Array.isArray(data.episodes)) {
          const ep = _pickAnimeEpisode(data.episodes, season, episode);
          if (!ep || !ep.url) return [];
          episodeUrl = ep.url;
        }

        // 3. /play?url= → map data[].url to plain strings.
        const pr = await fetch(`${base}/play?url=${encodeURIComponent(episodeUrl)}`);
        if (!pr.ok) return [];
        const pj = await pr.json();
        return (pj && pj.status === 'success' && Array.isArray(pj.data))
          ? pj.data.map(o => o && o.url).filter(Boolean)
          : [];
      }

      // 'legacy' — title + season/episode in one /play call.
      if (!matchTitle) return [];
      const u = `${base}/play?title=${encodeURIComponent(matchTitle)}&season=${encodeURIComponent(season)}&episode=${encodeURIComponent(episode)}`;
      const r = await fetch(u);
      if (!r.ok) return [];
      const j = await r.json();
      return (j && j.status === 'success' && Array.isArray(j.embed_links)) ? j.embed_links.filter(Boolean) : [];
    } catch (_) { return []; }
  }

  function appendWorkerSources(worker, links) {
    if (!links || !links.length || !mini.activePlayer) return;
    const tag = (worker.lang === 'dub') ? 'Dub' : 'Sub';
    const extra = links.map((url, i) => ({ name: `${worker.name} ${i + 1} (${tag})`, kind: 'embed', url }));
    mini.activePlayer.appendSources(extra);
  }

  // For an anime context: if the user already matched this worker, replay from
  // the remembered URL. Otherwise auto-search the AniList title once and cache
  // the best candidate. The first Hindi worker that returns embeds becomes the
  // auto-playing primary source (MegaPlay stays folded under "Load more").
  function resolveMatchedAnimeWorkers(anilistId, episode, season) {
    const workers = Array.isArray(_CFG.ANIME_WORKERS) ? _CFG.ANIME_WORKERS : [];
    if (!workers.length) return;
    const remembered = _animeMatch[anilistId] = _animeMatch[anilistId] || {};
    const ctxTitle = (mini.activeCtx && mini.activeCtx.title) || '';
    const animeSeason = (season != null && season !== 1) ? season : _detectAnimeSeason(ctxTitle);
    let primaryClaimed = false;

    workers.forEach((w) => {
      (async () => {
        let match = remembered[w.name];

        if (!match && ctxTitle) {
          const baseTitle = _stripSeasonSlug(ctxTitle);
          const cands = await searchAnimeWorker(w, baseTitle);
          const pick  = _autoPickAnimeMatch(cands, baseTitle) || _autoPickAnimeMatch(cands, ctxTitle);
          if (pick) {
            match = { title: pick.title, url: pick.url || '' };
            remembered[w.name] = match;
            _saveAnimeMatches();
          }
        }
        if (!match) return;

        const cached = _getCachedEpisodeLinks(anilistId, episode, w.name);
        let links;
        if (Array.isArray(cached)) {
          links = cached;
        } else {
          links = await resolveAnimeWorker(w, match, animeSeason, episode);
          _cacheEpisodeLinks(anilistId, episode, w.name, links);
        }

        const live = mini.activePlayer && mini.activeCtx
          && mini.activeCtx.type === 'anime'
          && String(mini.activeCtx.id) === String(anilistId)
          && String(mini.activeCtx.episode) === String(episode);
        if (!live || !links.length) {
          _prefetchNextEpisode(w, match, anilistId, episode, animeSeason);
          return;
        }

        appendWorkerSources(w, links);

        if (!primaryClaimed) {
          primaryClaimed = true;
          try {
            const all = mini.activePlayer.listSources();
            const tag = (w.lang === 'dub') ? 'Dub' : 'Sub';
            const target = all.find(s => s.name === `${w.name} 1 (${tag})`);
            if (target && typeof mini.activePlayer.setSource === 'function') {
              mini.activePlayer.setSource(target.index);
            }
          } catch (_) {}
        }
  ...
  function _prefetchNextEpisode(worker, match, anilistId, episode, season) {
    const next = (parseInt(episode, 10) || 0) + 1;
    if (next <= 1) return;
    if (_getCachedEpisodeLinks(anilistId, next, worker.name)) return;
    setTimeout(() => {
      resolveAnimeWorker(worker, match, season, next)
        .then((links) => { _cacheEpisodeLinks(anilistId, next, worker.name, links || []); })
        .catch(() => {});
    }, 1500);
  }

  // Native "Pick the right title" modal — reuses the existing .source-modal /
  // .source-rows / .source-row chrome (no new CSS). The user selects the entry
  // that matches; we remember it and pull this episode's streams from it.
  async function openAnimeMatchModal(worker) {
    const ctx = mini.activeCtx;
    if (!ctx || ctx.type !== 'anime' || !worker) return;
    const title = ctx.title || '';
    const ep = ctx.episode || 1;

    const existing = $('#animeMatchModal');
    if (existing) existing.remove();

    const m = document.createElement('div');
    m.id = 'animeMatchModal';
    m.className = 'source-modal open';
    m.innerHTML = `
      <div class="source-modal__panel">
        <div class="source-modal__head">
          <h3 class="serif"><em>Pick</em> the right title</h3>
          <button class="icon-btn" data-close><i class="ri-close-line"></i></button>
        </div>
        <p>Choose the entry on ${html(worker.name)} that matches <b>${html(title)}</b>. Your pick is saved — every future episode resolves instantly.</p>
        <div class="anime-match-manual">
          <div class="anime-match-manual__label"><i class="ri-search-2-line"></i> Search a different title on ${html(worker.name)}</div>
          <div class="anime-match-manual__row">
            <span class="anime-match-manual__icon"><i class="ri-search-line"></i></span>
            <input type="text" id="animeManualTitle" placeholder="Type the title and press Enter…" value="${html(_stripSeasonSlug(title))}">
            <button class="btn-primary" id="animeManualGo"><i class="ri-arrow-right-line"></i> Search</button>
          </div>
          <div class="anime-match-manual__hint">Tip — workers index by base title. Drop "Season 2", "Part 3", "Cour 2" etc. for the cleanest hit rate.</div>
        </div>
        <div class="source-rows" id="animeMatchRows"></div>
        <div class="source-modal__hint"><i class="ri-search-line"></i> Manual match avoids wrong auto-detected results.</div>
      </div>`;
    document.body.appendChild(m);

    const close = () => { m.classList.remove('open'); setTimeout(() => m.remove(), 240); };
    m.querySelector('[data-close]').addEventListener('click', close);
    m.addEventListener('click', (e) => { if (e.target === m) close(); });

    const wireRowClicks = () => {
      $$('#animeMatchModal .source-row[data-title]').forEach((row) => {
        row.onclick = async () => {
          const match = { title: row.dataset.title, url: row.dataset.url || '' };
          _animeMatch[ctx.id] = _animeMatch[ctx.id] || {};
          _animeMatch[ctx.id][worker.name] = match;
          _saveAnimeMatches();

          row.querySelector('.source-row__sub').textContent = 'Resolving this episode…';
          const links = await resolveAnimeWorker(worker, match, _detectAnimeSeason(title), ep);
          close();

          const live = mini.activePlayer && mini.activeCtx
            && mini.activeCtx.type === 'anime' && String(mini.activeCtx.id) === String(ctx.id);
          if (!live) return;

          if (links.length) {
            appendWorkerSources(worker, links);
            toast(`Added ${links.length} ${worker.name} server${links.length > 1 ? 's' : ''}`);
            // Auto-switch to the newly added Hindi server.
            try {
              const all = mini.activePlayer.listSources();
              const tag = (worker.lang === 'dub') ? 'Dub' : 'Sub';
              const target = all.find(s => s.name === `${worker.name} 1 (${tag})`);
              if (target && mini.activePlayer.setSource) mini.activePlayer.setSource(target.index);
            } catch (_) {}
          } else {
            toast(`No streams found on ${worker.name} for E${ep}`);
          }
        };
      });
    };
    ...
    const renderRows = (results) => {
      const rows = $('#animeMatchRows', m);
      if (!rows) return;
      if (!results.length) {
        rows.innerHTML = `<button class="source-row" disabled>
          <div class="source-row__num"><i class="ri-error-warning-line"></i></div>
          <div class="source-row__body">
            <div class="source-row__name">No matches found</div>
            <div class="source-row__sub">Edit the title above and search again</div>
          </div></button>`;
        return;
      }
      rows.innerHTML = results.map((r, i) => `
        <button class="source-row" data-title="${html(r.title)}" data-url="${html(r.url || '')}">
          <div class="source-row__num">${r.poster
              ? `<img src="${html(r.poster)}" alt="" loading="lazy" style="width:100%;height:100%;object-fit:cover;border-radius:8px;display:block">`
              : String(i + 1).padStart(2, '0')}</div>
          <div class="source-row__body">
            <div class="source-row__name">${html(r.title)}</div>
            <div class="source-row__sub">${r.type === 'Movie' ? 'Movie' : 'Series'} · ${html(worker.name)}</div>
          </div>
          <i class="ri-arrow-right-s-line"></i>
        </button>`).join('');
      wireRowClicks();
    ...
    const runSearch = async (q) => {
      const rows = $('#animeMatchRows', m);
      rows.innerHTML = `<button class="source-row" disabled>
        <div class="source-row__num"><i class="ri-loader-4-line"></i></div>
        <div class="source-row__body">
          <div class="source-row__name">Searching ${html(worker.name)}…</div>
          <div class="source-row__sub">Looking up "${html(q)}"</div>
        </div></button>`;
      const r = await searchAnimeWorker(worker, q);
      renderRows(r);
    };

    const manualBtn = m.querySelector('#animeManualGo');
    const manualInp = m.querySelector('#animeManualTitle');
    manualBtn.addEventListener('click', () => { const q = manualInp.value.trim(); if (q) runSearch(q); });
    manualInp.addEventListener('keydown', (e) => { if (e.key === 'Enter') { e.preventDefault(); manualBtn.click(); } });

    runSearch(_stripSeasonSlug(title));
  }

  // ───── ANIME WATCH (MegaPlay · direct AniList) ─────────────────────────────
  // Bypasses TMDB / AniZip entirely. Pulls metadata straight from AniList's
  // GraphQL endpoint and hands the player an AniList ID + episode number;
  // MegaPlay's iframe handles every server / language / subtitle decision
  // internally. Reuses the existing .watch-page / .episodes-grid / .episode
  // structure so the UI is byte-for-byte identical to the movie/tv path.
  async function renderAnimeWatch(anilistId, episodeNum) {
    const id = +anilistId;
    ...
    try {
      ...
      media = json && json.data && json.data.Media ? json.data.Media : null;
    } catch (_) { media = null; }
    ...
    const totalEps = (Number.isFinite(media.episodes) && media.episodes > 0)
      ? media.episodes
      : (isMovie ? 1 : 12);
    const fmtLabel = ({ TV:'SERIES', TV_SHORT:'SHORT', MOVIE:'FILM', SPECIAL:'SPECIAL', OVA:'OVA', ONA:'ONA', MUSIC:'MUSIC' })[media.format] || 'ANIME';

    $('.watch-page__crumb').innerHTML =
      `<span>Anime</span><span class="sep">·</span><b>${html(title)}</b><span class="sep">·</span><span>E${ep}</span>`;

    $('.watch-page__info').innerHTML = `
      <div>
        <h1 class="watch-page__title">${html(title)} — <em>E${ep}</em></h1>
        <div class="watch-page__meta">
          <span>${year || ''}</span><span class="dot"></span>
          <span class="mono">${fmtLabel}</span>
          ${score ? `<span class="dot"></span><span style="color:var(--accent)"><i class="ri-star-fill"></i> ${score}</span>` : ''}
        </div>
        <p class="watch-page__overview">${collapsible(overview, 2, 180)}</p>
      </div>
      <aside class="watch-page__aside">
        <h4>What's next</h4>
        <div id="nextUp"><p style="color:var(--text-mute);font-size:13px">Up next loads as you watch.</p></div>
      </aside>
    `;

    const bg     = media.bannerImage || media.coverImage?.large || '';
    const poster = media.coverImage?.large || '';
    const lbg = loaderBgEl();
    if (bg && lbg) lbg.style.backgroundImage = `url(${bg})`;
    const host = playerHost();
    if (host) {
      let art = host.querySelector('.player-host__loader-art');
      if (!art) {
        const loaderEl = host.querySelector('.player-host__loader');
        if (loaderEl) {
          art = document.createElement('div');
          art.className = 'player-host__loader-art';
          loaderEl.insertBefore(art, loaderEl.querySelector('.player-host__loader-spinner'));
        }
      }
      if (art) {
        art.innerHTML = poster
          ? `<div class="player-host__loader-poster"><img src="${poster}" alt="" loading="lazy"><div class="player-host__loader-shimmer"></div></div>
             <div class="player-host__loader-name">${html(title)}</div>
             <div class="player-host__loader-ep">Episode ${ep}</div>`
          : `<div class="player-host__loader-name">${html(title)}</div>
             <div class="player-host__loader-ep">Episode ${ep}</div>`;
      }
    }
    if (media.coverImage?.color && perShowAccentEnabled()) applyPerShowAccent(media.coverImage.color);

    const watchPage = $('.watch-page');
    if (watchPage) {
      watchPage.insertAdjacentHTML('beforeend', `
        <div class="watch-page__episodes">
          <section class="section section--episodes">
            <header class="section__head">
              <div>
                <h2 class="section__title">Choose an <em>episode</em></h2>
                <div class="section__sub" id="anEpisodesSub">${totalEps} episode${totalEps>1?'s':''} · ${fmtLabel.toLowerCase()}</div>
              </div>
            </header>
            <div class="an-season-pills" id="anSeasonPills" hidden></div>
            <div class="episodes-grid episodes-grid--compact" id="episodesGrid"></div>
          </section>
        </div>
      `);

      const grid = $('#episodesGrid');
      const subEl = $('#anEpisodesSub');

      const renderEpsFor = (anilistId, eps, currentEp, label) => {
        if (!eps || eps < 1) {
          grid.innerHTML = '<p class="an-empty" style="padding:24px;color:var(--text-mute)">No episodes listed for this entry yet.</p>';
          if (subEl) subEl.textContent = `${label || ''} · upcoming`;
          return;
        }
        grid.innerHTML = Array.from({ length: eps }, (_, i) => {
          const n = i + 1;
          const isCurrent = (Number(anilistId) === Number(id) && n === ep);
          const playingCls = isCurrent ? ' episode--playing' : '';
          return `
            <a class="episode episode--compact${playingCls}" href="/watch/anime/${anilistId}/${n}" data-link>
              <div class="episode__numbox">
                <span class="episode__num">E${n}</span>
                <span class="episode__playhint"><i class="ri-play-fill"></i></span>
              </div>
              <div class="episode__body">
        ...
        if (subEl) subEl.textContent = `${eps} episode${eps>1?'s':''}${label ? ' · ' + label : ''}`;
      };

      renderEpsFor(id, totalEps, ep, title);

      (async () => {
        try {
          const chain = await _buildSeasonChain(id, media);
          if (!chain || chain.length < 2) return;

          const pillsEl = $('#anSeasonPills');
          if (!pillsEl) return;
          pillsEl.hidden = false;

          let activeIdx = chain.findIndex(s => Number(s.id) === Number(id));
          ...
          const commonPrefix = (() => {
            ...
            for (let i = 1; i < titles.length; i++) {
              while (p && titles[i].toLowerCase().indexOf(p.toLowerCase()) !== 0) p = p.slice(0, -1);
              if (!p) break;
            }
            return p.replace(/[\s:·\-–—]+$/, '');
          })();
          ...
          const renderPills = () => {
            pillsEl.innerHTML = chain.map((s, i) => {
              const isActive = i === activeIdx;
              let label = s.title || `Part ${i+1}`;
              if (commonPrefix && commonPrefix.length > 4) {
                const stripped = label.slice(commonPrefix.length).replace(/^[\s:·\-–—]+/, '');
                if (stripped) label = stripped;
              }
              return `<button class="an-season-pill ${isActive ? 'is-active' : ''}" data-i="${i}" title="${html(s.title || '')}">
                <span class="an-season-pill__n">${i + 1}</span>
                <span class="an-season-pill__name">${html(label)}</span>
                <span class="an-season-pill__count">${s.episodes ? s.episodes + ' ep' : (s.format === 'MOVIE' ? 'Film' : '?')}</span>
              </button>`;
            }).join('');
          };
          renderPills();
          ...
          pillsEl.addEventListener('click', (e) => {
            const btn = e.target.closest('.an-season-pill');
            if (!btn) return;
            const i = +btn.dataset.i;
            if (i === activeIdx) return;
            activeIdx = i;
            renderPills();
            const s = chain[activeIdx];
            renderEpsFor(s.id, s.episodes, null, s.title);
          });

          requestAnimationFrame(() => {
            const active = pillsEl.querySelector('.an-season-pill.is-active');
    ...
    const sameEp = mini.activePlayer && mini.activeCtx
      && mini.activeCtx.type === 'anime'
      && String(mini.activeCtx.id) === String(id)
      && String(mini.activeCtx.episode) === String(ep);

    if (!sameEp) {
      if (mini.activePlayer) { try { mini.activePlayer.destroy(); } catch (_) {} mini.activePlayer = null; }

      // MegaPlay plays instantly off the AniList id (Sub + Dub). The scraper
      // workers (AnimeSalt / ToonStream) search by title and take a moment, so
      // we start playback immediately and append their embeds as they resolve.
      const animeServers = [
      ...
      mini.activePlayer = new StellarPlayer('#playerFrame', {
        ...
        onLoading: (loading, name, status) => mini.setLoading(loading, name, status),
        onSourceChange: (name) => { if ($('#sourceName')) $('#sourceName').textContent = name; },
        onSourcesChange: () => { if ($('#sourceModal')) { $('#sourceModal').remove(); openSourceModal(); } },
        onEvent: () => {},
        onProgress: () => {}
      });
      mini.activePlayer.playAnime(id, ep);
      mini.activeCtx = {
        type: 'anime', id, episode: ep,
        title, year,
        poster: media.coverImage?.large || '',
        backdrop: media.bannerImage || ''
      };

      // Auto-search workers by AniList title (cached per anime+worker). If a
      // worker returns embeds, the FIRST one auto-plays as the primary stream.
      // MegaPlay stays in the folded "Load more" group.
      resolveMatchedAnimeWorkers(id, ep, 1);
    } else {
      mini.activeCtx.title = title;
      resolveMatchedAnimeWorkers(id, ep, 1);
    }
    ...
    minBtn.hidden = !pipEnabled();
    minBtn.addEventListener('click', () => {
      if (!pipEnabled()) { toast('Mini player is off · turn it on in Settings', 'picture-in-picture-2-line'); return; }
      mini.toMini();
    });
    const openSource = () => openSourceModal();
    $('#watchSourceBtn').addEventListener('click', openSource);
  ...
  function openSourceModal() {
    ...
    m.id = 'sourceModal';
    m.className = 'source-modal open';
    // ── Display-only grouping (NO playback logic changes) ───────────────────
    // setSource() still uses each source's REAL index. We only reorder HOW the
    // rows are presented:
    //   • Anime: scraper-worker streams (AnimeSalt / ToonStream) are the
    //     primary list; MegaPlay (TMDB embed) streams are NOT premier — they
    //     fold away under a collapsed "Load more servers" section.
    //   • Movie / TV: every source shows in one list (nothing folded).
    const allSources = mini.activePlayer.listSources();
    const curName    = mini.activePlayer.currentSourceName();
    const isAnime    = mini.activeCtx && mini.activeCtx.type === 'anime';
    const isMega     = (s) => /megaplay/i.test(s.name);
    const primary    = isAnime ? allSources.filter(s => !isMega(s)) : allSources;
    const folded     = isAnime ? allSources.filter(s =>  isMega(s)) : [];

    const rowHtml = (s, idx) => `
      <button class="source-row src-row ${s.name === curName ? 'active' : ''}" data-i="${s.index}">
        <div class="source-row__num">${s.name === curName ? '<i class="ri-volume-up-fill"></i>' : String(idx + 1).padStart(2, '0')}</div>
        <div class="source-row__body">
          <div class="source-row__name">${html(s.name)}</div>
          <div class="source-row__sub">${s.name === curName ? 'Now playing' : 'Tap to switch'}</div>
        </div>
        <i class="ri-${s.name === curName ? 'check-line' : 'arrow-right-s-line'}"></i>
      </button>`;

    const primaryRows = primary.map(rowHtml).join('');
    const foldedRows  = folded.map(rowHtml).join('');

    // Anime-only: manual "pick the matching title" rows for each scraper worker.
    const animeWorkers = isAnime && Array.isArray(_CFG.ANIME_WORKERS) ? _CFG.ANIME_WORKERS : [];
    let matchRows = '';
    if (animeWorkers.length) {
      const matched = _animeMatch[mini.activeCtx.id] || {};
      matchRows = animeWorkers.map((w) => {
        const isMatched = !!matched[w.name];
        const tag = (w.lang === 'dub') ? 'Dub' : 'Sub';
        return `
          <button class="source-row src-row src-row--match ${isMatched ? 'is-matched' : ''}" data-anime-match="${html(w.name)}">
            <div class="source-row__num"><i class="ri-${isMatched ? 'refresh-line' : 'search-line'}"></i></div>
            <div class="source-row__body">
              <div class="source-row__name">${html(w.name)} (${tag})</div>
              <div class="source-row__sub">${isMatched ? 'Matched · tap to re-pick title' : 'Tap once to choose the matching title'}</div>
            </div>
            <i class="ri-arrow-right-s-line"></i>
          </button>`;
      }).join('');
    ...
    m.innerHTML = `
      <div class="source-modal__panel">
        <div class="source-modal__head">
          ...
          <span class="src-count" title="Available servers"><i class="ri-stack-line"></i> ${total} server${total !== 1 ? 's' : ''}</span>
          <button class="icon-btn" data-close aria-label="Close"><i class="ri-close-line"></i></button>
        </div>
        <p class="src-intro">Streams switch the moment you pick one. You can choose manually anytime.</p>

        ${primaryRows ? `
          <div class="src-group">
            <div class="src-group__label">${isAnime ? 'Primary servers' : 'Servers'} <span class="src-pill">${primary.length}</span></div>
            <div class="source-rows">${primaryRows}</div>
          </div>` : ''}

        ${matchRows ? `
          <div class="src-group">
            <div class="src-group__label"><i class="ri-links-line"></i> Add Sub / Dub <span class="src-pill">${animeWorkers.length}</span></div>
            <div class="source-rows">${matchRows}</div>
          </div>` : ''}

        ${foldedRows ? `
          <div class="src-group src-fold" id="srcFold">
            <button class="src-fold__toggle" type="button" id="srcFoldToggle" aria-expanded="false">
              <i class="ri-add-circle-line src-fold__plus"></i>
              <span>Load more servers</span>
              <span class="src-pill">${folded.length}</span>
              <i class="ri-arrow-down-s-line src-fold__chev"></i>
            </button>
            <div class="source-rows src-fold__body" id="srcFoldBody" hidden>${foldedRows}</div>
          </div>` : ''}
    ...
    const closeModal = () => { m.classList.remove('open'); setTimeout(() => m.remove(), 240); };
    m.querySelector('[data-close]')?.addEventListener('click', closeModal);

    const foldToggle = $('#srcFoldToggle', m);
    if (foldToggle) {
      foldToggle.addEventListener('click', () => {
        const body = $('#srcFoldBody', m);
        const willOpen = body.hasAttribute('hidden');
        if (willOpen) body.removeAttribute('hidden'); else body.setAttribute('hidden', '');
        $('#srcFold', m).classList.toggle('is-open', willOpen);
        foldToggle.setAttribute('aria-expanded', String(willOpen));
      });
    }

    $$('#sourceModal .source-row[data-anime-match]').forEach((btn) => {
      btn.addEventListener('click', () => {
        const wname = btn.dataset.animeMatch;
        const worker = (Array.isArray(_CFG.ANIME_WORKERS) ? _CFG.ANIME_WORKERS : []).find(w => w.name === wname);
        if (!worker) return;
        closeModal();
        openAnimeMatchModal(worker);
      });
    });
    $$('#sourceModal .source-row[data-i]').forEach(row => {
  ...
  function openSearch(scope='all') {
    ...
    input.placeholder = scope === 'anime'
      ? 'Search anime…'
      : (scope === 'movie' ? 'Search films, series, people…' : 'Search films, series, anime, people…');
    input.focus();
    if (!input.value) populateSearchSuggest(); else onSearch();
  }
  ...
  // AniList search — the dedicated Anime search bar queries AniList directly so
  // anime results are real anime entries (not TMDB look-alikes). Cards open the
  // anime watch page via the existing /watch/anime/<id>/1 route.
  async function searchAniList(q) {
    try {
          ...
          query: `query ($search: String) {
            Page(perPage: 24) {
              media(type: ANIME, search: $search, sort: SEARCH_MATCH, isAdult: false) {
                id title { romaji english } coverImage { large } format seasonYear averageScore
              }
            }
          }`,
          variables: { search: q }
      ...
      return (j && j.data && j.data.Page && Array.isArray(j.data.Page.media)) ? j.data.Page.media : [];
    } catch (_) { return []; }
  }

  function animeSearchCard(mm) {
    const t = (mm.title && (mm.title.english || mm.title.romaji)) || 'Unknown';
    const poster = (mm.coverImage && mm.coverImage.large) || '';
    const year = mm.seasonYear || '';
    const isFilm = mm.format === 'MOVIE';
    const rating = mm.averageScore ? `<span class="title-card__rating"><i class="ri-star-fill"></i> ${(mm.averageScore / 10).toFixed(1)}</span>` : '';
    const art = poster
      ? `<img src="${poster}" alt="${html(t)}" loading="lazy" decoding="async">`
      : `<div class="title-card__placeholder"><i class="ri-${isFilm ? 'film-line' : 'tv-2-line'}"></i></div>`;
    return `<a class="title-card" href="/watch/anime/${mm.id}/1" data-link onclick="OMNIFLIX.closeSearch()">
      <div class="title-card__poster"><div class="title-card__shimmer"></div>${art}${rating}<span class="title-card__badge">${isFilm ? 'Film' : 'Series'}</span></div>
      <div class="title-card__meta">
        <div class="title-card__title">${html(t)}</div>
        <div class="title-card__sub">${year || '—'}</div>
      </div>
    </a>`;
  }
  ...
  const onSearch = debounce(async () => {
    ...
    const scope = $('#searchInput').dataset.scope || 'all';

    // Dedicated Anime search → AniList only.
    if (scope === 'anime') {
      const media = await searchAniList(q);
      if (!media.length) {
        $('#searchResults').innerHTML = `<div class="empty"><div class="empty__icon"><i class="ri-search-line"></i></div><h3 class="serif"><em>No anime found.</em></h3><p>Try the romaji or English title.</p></div>`;
        return;
      }
      $('#searchResults').innerHTML = `<div class="group group--anime"><h4><i class="ri-sparkling-2-fill" style="color:var(--accent)"></i> Anime · ${media.length}</h4><div class="browse-grid">${media.map(animeSearchCard).join('')}</div></div>`;
      return;
    }
    ...
    $('#searchResults').innerHTML = `
      ${(anime.length && (($('#searchInput').dataset.scope || 'all') !== 'movie')) ? `<div class="group group--anime"><h4><i class="ri-sparkling-2-fill" style="color:var(--accent)"></i> Anime · ${anime.length}</h4><div class="browse-grid">${anime.slice(0, 12).map(titleCard).join('')}</div></div>` : ''}
  ...
  $('#searchBtn').addEventListener('click', () => openSearch('movie'));
  $('#animeSearchBtn')?.addEventListener('click', () => openSearch('anime'));
  ...
  $('#themeBtn').addEventListener('click', () => {
    ...
    refreshPipToggle();
    refreshAmoledToggle();
    refreshPerShowAccentToggle();
    $('#themeSheet').classList.add('open');
  });

  function refreshAmoledToggle() {
    const sw = $('#amoledToggle');
    if (!sw) return;
    const on = amoledEnabled();
    sw.setAttribute('aria-checked', on ? 'true' : 'false');
    sw.classList.toggle('switch--on', on);
    $('#amoledRow')?.classList.toggle('setting-row--off', !on);
  }
  $('#amoledToggle')?.addEventListener('click', () => {
    const next = !amoledEnabled();
    setAmoledEnabled(next);
    refreshAmoledToggle();
    toast(next ? 'AMOLED black on' : 'AMOLED black off', next ? 'contrast-2-line' : 'contrast-line');
  });
  refreshAmoledToggle();

  function refreshPerShowAccentToggle() {
    const sw = $('#perShowAccentToggle');
    if (!sw) return;
    const on = perShowAccentEnabled();
    sw.setAttribute('aria-checked', on ? 'true' : 'false');
    sw.classList.toggle('switch--on', on);
    $('#perShowAccentRow')?.classList.toggle('setting-row--off', !on);
  }
  $('#perShowAccentToggle')?.addEventListener('click', () => {
    const next = !perShowAccentEnabled();
    setPerShowAccentEnabled(next);
    refreshPerShowAccentToggle();
    toast(next ? 'Auto-tint on' : 'Auto-tint off', 'palette-line');
  });
  refreshPerShowAccentToggle();
